...
def billing_dashboard(request):
    ...
    # 🟢 NEW: Fetch Billing Profile to show Wallet Balance
    from apps.saas_core.services.billing_profile_service import BillingProfileService
    billing_profile = BillingProfileService.get_or_create_profile(workspace=workspace)
    ...
    context = {
        ...
        'workspace': workspace,
        'invoices': invoices,
        'billing_profile': billing_profile # Passed to template
    }
...
def checkout_summary(request, price_id):
    ...
    if request.method == 'POST':
        try:
            ...
            # 🟢 FOOLPROOF REDIRECT: Catch any Zero-Dollar or Completed Order
            if order.status in [OrderStatus.COMPLETED, 'COMPLETED'] or order.total_amount <= 0:
                messages.success(request, "Success! Your plan was activated automatically using your credits.")
...
def download_invoice_pdf(request, invoice_id):
    ...
    # Generate PDF if it doesn't exist or if it's corrupted (very small size)
    if not invoice.pdf_file or invoice.pdf_file.size < 1000:
        InvoiceService.generate_pdf(invoice)
        
    try:
        # FileResponse handles streaming, closing, and EOF pointers automatically!
        response = FileResponse(invoice.pdf_file.open('rb'), content_type='application/pdf')
        response['Content-Disposition'] = f'inline; filename="{invoice.invoice_number}.pdf"'
        return response
    except Exception as e:
        messages.error(request, "Failed to load PDF document.")
        return redirect('saas_core:billing_dashboard')  